...
<template>
  <van-swipe-cell :disabled="task.isCompleted">
    <div class="task-card" :class="{ completed: task.isCompleted }" @click="goDetail">
      <van-checkbox :model-value="task.isCompleted" @click.stop @change="handleToggle" />
      <div class="task-body">
      ...
      <div class="task-right">
        ...
        <span class="sub-count" v-if="task.subTasks?.length">{{ task.subTasks.filter(s => s.isCompleted).length }}/{{ task.subTasks.length }}</span>
      </div>
    </div>
    <template #right>
...
.task-right { display: flex; flex-direction: column; align-items: flex-end; gap: 4px; min-width: 24px; }
