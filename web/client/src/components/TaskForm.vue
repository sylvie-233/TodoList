...
const emit = defineEmits<{
  submit: [dto: CreateTaskDto & Partial<UpdateTaskDto>];
}>();
...
const selectedListName = computed(() => {
  ...
  const lst = listStore.lists.find((l) => l.id === form.listId);
  return lst?.name ?? '';
});
...
const priorityColumns = [
  { text: '无', value: Priority.NONE },
  { text: '低', value: Priority.LOW },
  { text: '中', value: Priority.MEDIUM },
  { text: '高', value: Priority.HIGH },
  { text: '紧急', value: Priority.URGENT },
];

const priorityLabel = computed(
  () => priorityColumns.find((p) => p.value === form.priority)?.text ?? '无',
);
...
watch(
  ...
  (task) => {
    if (task) {
      ...
      form.listId = task.listId ?? undefined;
